...
public class RcsSettingsUtils {
    ...
    public String readParameter(String str) {
        ...
        Cursor query = this.mContext.getContentResolver().query(sDatabaseUri, (String[]) null, WHERE_CLAUSE, new String[]{str}, (String) null);
        if (query != null) {
            try {
                if (query.moveToFirst()) {
                    String string = query.getString(query.getColumnIndexOrThrow("value"));
                    Log.d(this.LOG_TAG, String.format("readParameter: %s = %s", new Object[]{str, string}));
                    query.close();
                    return string;
                }
            } catch (Throwable th) {
    ...
    public int writeParameter(String str, String str2) {
        ...
        IMSLog.s(str3, "writeParameter: " + str + AuthenticationHeaders.HEADER_PRARAM_SPERATOR + str2);
        ...
        try {
            ...
            Log.e(str4, "SQL exception while insert Settings Parameter. " + e2);
            return 0;
        }
    }
    ...
    public void loadCCAndAC() {
        ...
        if (strArr == null) {
            ...
            Log.d(str, "there is no country code info about this country: " + simCountryIso);
            return;
        }
        String str2 = strArr[0];
        String str3 = this.LOG_TAG;
        Log.d(str3, "country code " + str2);
        if (!TextUtils.isEmpty(str2)) {
            setCountryCode(COUNTRY_CODE_PREFIX.concat(str2));
        }
        if (strArr.length > 1) {
            String str4 = strArr[1];
            String str5 = this.LOG_TAG;
            Log.d(str5, "country area code " + str4);
            if (!TextUtils.isEmpty(str4)) {
                setCountryAreaCode(str4);
            }
        }
    }
    ...
    public void updateSettings() {
        ...
        for (ImsRegistration impuList : registrationInfo) {
            for (NameAddr uri : impuList.getImpuList()) {
                ImsUri uri2 = uri.getUri();
                String extractNumberFromUri = PhoneUtils.extractNumberFromUri(uri2.toString());
                if (extractNumberFromUri != null) {
                    setMyContactId(extractNumberFromUri);
                    IMSLog.s(this.LOG_TAG, String.format("Load Number: %s from uri: %s", new Object[]{extractNumberFromUri, uri2.toString()}));
                    return;
                }
            }
        }
    ...
    public void updateTapiSettings() {
        ...
        boolean z = true;
        if (DmConfigHelper.getImsSwitchValue(this.mContext, DeviceConfigManager.RCS, SimUtil.getActiveDataPhoneId()) != 1) {
            z = false;
        }
        String str = this.LOG_TAG;
        IMSLog.s(str, "check isRcsEnabled:" + z);
    ...
    private class SimEventListener implements ISimEventListener {
        ...
        public void onReady(int i, boolean z) {
            String r0 = RcsSettingsUtils.this.LOG_TAG;
            Log.d(r0, "onReady: subId=" + i + " absent=" + z);
            if (!z) {
